...
const storeKey = computed(() => (board.value ? `beecoding.livecode.${board.value.id}` : ''));
...
async function copyLecture() {
  const text = lecture.value?.code || '';
...
watch(code, () => {
  pushSoon();
  try { if (storeKey.value && isStaff.value) localStorage.setItem(storeKey.value, code.value); } catch { /* ignore */ }
});
  ...
  <div v-if="board" class="h-full flex flex-col">
    <div class="flex items-center gap-3 px-4 py-2 border-b border-slate-200 dark:border-slate-800 flex-wrap">
      ...
      <div class="ml-auto flex items-center gap-2">
        ...
        <span v-if="isStaff" class="inline-flex rounded-lg border border-slate-300 dark:border-slate-700 overflow-hidden text-xs">
    ...
    <!-- Student: read-only follow-along -->
    <div v-else class="flex-1 min-h-0 flex flex-col">
      <div v-if="lecturingOn && lecture" class="flex items-center gap-2 px-4 py-1.5 text-xs bg-sky-50 dark:bg-sky-500/10 text-sky-700 dark:text-sky-300">
        <span>👨‍🏫 {{ lecture.teacherName }} · live · {{ lecture.language === 'c' ? 'C' : 'C++' }} · {{ ago(lecture.updatedAt) }} ago</span>
        <button @click="copyLecture" class="ml-auto bg-sky-600 hover:bg-sky-700 text-white rounded-lg px-2.5 py-0.5">
          {{ copied ? 'Copied ✓' : 'Copy code' }}
        </button>
      </div>
      <div v-if="lecturingOn && lecture" class="flex-1 min-h-0">
        <MonacoEditor :model-value="lecture.code" :language="lecture.language" :read-only="true" />
      </div>
      <div v-else class="flex-1 grid place-items-center text-sm text-slate-400 dark:text-slate-500 p-8 text-center">
        {{ lecturingOn ? 'Waiting for the teacher to start typing…' : 'The teacher hasn’t started a live session yet.' }}
      </div>
    </div>
  </div>
</template>
