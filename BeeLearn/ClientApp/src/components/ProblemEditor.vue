...
<template>
  <div class="fixed inset-0 bg-black/40 flex items-start justify-center p-4 overflow-y-auto z-50">
    <div class="bg-white rounded-xl w-full max-w-2xl p-5 my-8">
      <h2 class="font-bold text-lg mb-3">{{ props.problem?.id ? 'Edit' : 'New' }} problem</h2>
      <div class="space-y-3">
        <input v-model="form.title" placeholder="Title" class="w-full border border-slate-300 rounded-lg px-3 py-2" />
        <textarea v-model="form.statementMarkdown" placeholder="Statement (Markdown)" rows="4"
                  class="w-full border border-slate-300 rounded-lg px-3 py-2 font-mono text-sm"></textarea>
        <div class="flex gap-3 flex-wrap text-sm">
          <label class="flex items-center gap-1">Language
            <select v-model="form.language" class="border border-slate-300 rounded px-2 py-1">
              <option value="cpp">C++</option><option value="c">C</option>
            </select>
          </label>
          <label class="flex items-center gap-1">Time (ms)
            <input v-model.number="form.timeLimitMs" type="number" class="w-24 border border-slate-300 rounded px-2 py-1" />
          </label>
          <label class="flex items-center gap-1">Memory (KB)
            <input v-model.number="form.memoryLimitKb" type="number" class="w-28 border border-slate-300 rounded px-2 py-1" />
          </label>
          <label class="flex items-center gap-1">Position
            <input v-model.number="form.position" type="number" class="w-16 border border-slate-300 rounded px-2 py-1" />
          </label>
        </div>
        ...
        <div>
          ...
          <div v-for="(t, i) in form.testCases" :key="i" class="border border-slate-200 rounded-lg p-2 mb-2">
            <div class="grid grid-cols-2 gap-2">
              <textarea v-model="t.stdin" placeholder="stdin" rows="2"
                        class="border border-slate-300 rounded px-2 py-1 font-mono text-xs"></textarea>
            ...
            <div class="flex items-center gap-3 text-xs mt-1">
              ...
              <label class="flex items-center gap-1">points <input v-model.number="t.points" type="number" class="w-14 border border-slate-300 rounded px-1" /></label>
        ...
        <div class="flex gap-2 pt-1">
          ...
          <button @click="emit('cancel')" class="text-slate-500 px-3">Cancel</button>
          <button v-if="props.problem?.id" @click="del" class="text-red-600 ml-auto px-3">Delete</button>
        </div>
      </div>
    </div>
  </div>
