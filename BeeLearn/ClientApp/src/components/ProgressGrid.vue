    ...
    <table class="w-full text-sm border-collapse">
      <thead>
        <tr class="bg-slate-50 text-slate-500">
          <th class="text-left font-medium px-3 py-2 sticky left-0 bg-slate-50 z-10">Student</th>
      ...
      <tbody>
        <tr v-for="s in students" :key="s.userId" class="border-t border-slate-100">
          <td class="px-3 py-2 sticky left-0 bg-white z-10 whitespace-nowrap">
            ...
            <span v-if="s.userId === currentUserId" class="text-xs text-slate-400"> (you)</span>
          ...
          <td v-for="p in problems" :key="p.id" class="px-2 py-2 text-center">
            <template v-if="cell(s.userId, p.id)">
              <span v-if="cell(s.userId, p.id).redacted"
                    class="inline-block w-2.5 h-2.5 rounded-full bg-slate-300" title="attempted (hidden)"></span>
              ...
              <div v-if="!cell(s.userId, p.id).redacted && cell(s.userId, p.id).attempts > 1"
                   class="text-[10px] text-slate-400">×{{ cell(s.userId, p.id).attempts }}</div>
            </template>
        ...
        <tr v-if="!students.length">
          <td :colspan="problems.length + 2" class="px-3 py-6 text-center text-slate-400">
            No students yet.
          </td>
        </tr>
      </tbody>
