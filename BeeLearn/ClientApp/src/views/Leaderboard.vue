...
<template>
  <div class="max-w-3xl mx-auto px-4 py-8">
    <h1 class="text-xl font-bold mb-4">Papan Peringkat</h1>
    ...
    <div class="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
      <div v-for="r in rows" :key="r.userId"
        ...
        <span class="flex-1 text-sm font-medium truncate">
          {{ r.displayName }}
          <span v-if="r.me" class="text-xs text-amber-600 dark:text-amber-400"> (kamu)</span>
        </span>
        <span v-if="r.role === 'Teacher'" class="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400">guru</span>
      ...
      <p v-if="!rows.length" class="px-4 py-6 text-sm text-slate-400 dark:text-slate-500">
        Belum ada yang mengumpulkan XP. Kerjakan soal di <RouterLink to="/practice" class="text-amber-600">Latihan</RouterLink>.
      </p>
    </div>
  </div>
</template>
