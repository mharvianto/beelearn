...
<template>
  <div class="max-w-5xl mx-auto px-4 py-8">
    <div class="flex items-center justify-between mb-1">
      <h1 class="text-xl font-bold">Latihan</h1>
      <div class="text-sm text-slate-500 dark:text-slate-400">
        Lv {{ progress.level }} · {{ progress.xp }} XP · {{ progress.toNext }} XP ke level berikutnya
      </div>
    </div>
    <p class="text-sm text-slate-400 dark:text-slate-500 mb-4">
      Kerjakan soal apa saja. Menyelesaikan sebuah soal untuk pertama kali memberi XP
    ...
    <div class="flex gap-2 mb-4 flex-wrap">
      ...
      <select v-model="status" @change="load"
        ...
        <option value="unsolved">Belum selesai</option>
        <option value="attempted">Pernah dicoba</option>
        <option value="solved">Sudah selesai</option>
      </select>
    ...
    <p class="text-xs text-slate-400 dark:text-slate-500 mb-2">
      {{ items.length }} soal · {{ solvedCount }} selesai
    </p>
    ...
    <div class="border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden divide-y divide-slate-100 dark:divide-slate-800">
      ...
      <p v-if="!items.length" class="px-4 py-6 text-sm text-slate-400 dark:text-slate-500">Tidak ada soal.</p>
    </div>
  </div>
</template>
